...
### [x] 2-4. 연습 모드 / 힌트 모드 — **구현 완료 (2026-05-21, 런타임 테스트 대기)**

> solve 상단 ✋내가 풀기 토글 → 큐브 직접 드래그로 다음 무브 시도. **정답만 적용**,
> 틀리면 안내 + 💡힌트. 180°는 같은 방향 쿼터 2회. 드래그 로직은 공유 훅
> `use-cube-drag` 로 분리(매뉴얼 입력은 미변경, 중복은 추후 DRY 정리). 상세: `HANDOVER_v8.md` §3-3.
...
### [x] 5-2. 솔루션 단축 — **수준 선택으로 재구성 (3차, 2026-05-21)**

> 단순 알고리즘 교체 대신 **사용자 목적 선택 → 풀이 분기**로 구현.
> 홈에서 "차근차근 배우기"(LBL ~98수, 이해 학습) / "빠르게 풀이 보기"(Thistlethwaite ~31수).
> 옵션 C(이미 있던 solve_thistlethwaite 활용)를 학습 UX로 녹임. 출력은 면 무브만이라
> 시각화 호환. fast 실패 시 learn 폴백. 상세: `HANDOVER_v6.md`.

**목표**: 평균 ~100수 → ~50~70수 로 단축 (학습 부담 ↓) → **fast 모드 평균 ~31수로 초과 달성.**
