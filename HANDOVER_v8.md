...
작성일: **2026-05-21** (i18n·a11y·DRY 추가: **2026-05-23**)
대상 브랜치: `main` (private repo `netizen1119/rubiks-app-private`)
범위: 스캔→solve 흐름 안정화 / 나가기·i18n / **2-4 연습(힌트) 모드** /
**한·영 토글 i18n** / **a11y 보강** / **드래그 로직 DRY 통합**.
...
2026-05-23 에 **next-intl 기반 한국어/영어 토글**, **a11y 속성 보강**,
**매뉴얼 입력 드래그 로직의 use-cube-drag 훅 통합(DRY)** 까지 마쳤다.
...
- **a11y** — 토글 그룹 role/aria-pressed, 단계 dot progressbar, tutorial dialog + Esc
- **DRY** — 매뉴얼 입력 드래그를 use-cube-drag 훅으로 통합 (505 → 275줄)
...
| `1936aea` | add camera scan color calibration + auto face-orientation recognition | 커밋됨 |
| `31f9b6c` | add Korean/English i18n toggle and 2-4 practice mode | 커밋됨 (push 완료) |
| `a5b000b` | add i18n key/placeholder/empty-value test guard and bundle-size baseline | 커밋됨 |
| `84e8ce0` | add a11y attributes to solve toggles, stage progress, tutorial dialog | 커밋됨 |
| `1971c61` | DRY: route manual-input drag through useCubeDrag hook | 커밋됨 |
...
### 3-3. 한·영 i18n 토글 (`31f9b6c`, `a5b000b`, 2026-05-23)
...
- 한국어 홈 헤딩은 영문 직역("스캔. 풀기. 뿌듯.")이 어색해 의역 — "3D로 배우는 / 큐브 풀이."
  로 교체. 영문은 "Scan. Solve. Be proud." 유지 (heading.tsx 가 빈 라인 자동 필터).
- 영문 표현 일부는 다듬었지만 (cubing 표준 OLL/PLL/Sune 표기 등) 자연스러움은 여전히
  보강 여지 있음 → 후속 PR로.
- **i18n 가드 테스트** (`lib/i18n/messages.test.ts`, `a5b000b`): ko/en 키 동일성,
  키별 placeholder 일치, 의도되지 않은 빈 값 검출 (prefix/suffix·headingLine3 예외 허용).
  `package.json` test 스크립트가 `lib/i18n/*.test.ts` 포함하도록 확장.
- **dead code 정리** (`31f9b6c`에서 함께): `lib/maps/stage-descriptions.ts` 삭제,
  `cubeSidesFull/cubeSidesNamedColors` exports 제거, solver의 `stageName/stageNameEn`
  필드 제거.

### 3-4. a11y 보강 (`84e8ce0`, 2026-05-23)
- **solve 모드 토글** (📚/⚡): wrapper `role="group"` + `aria-label`, 각 버튼 `aria-pressed`.
- **연습 토글** (✋/👁): `aria-pressed` + `aria-label`.
- **자동재생 속도 그룹**: wrapper `role="group"` + 각 버튼 `aria-pressed`.
- **StageProgress**: dot row 가 `role="progressbar"` + `aria-valuemin/max/now/text`.
  fast 모드는 max=4, learn은 max=8. dot 자체는 `aria-hidden` (bar 가 의미 노출).
- **TutorialOverlay**: `role="dialog"` + `aria-modal="true"` + `aria-labelledby`.
  Esc 키로 닫기, 열림 시 close 버튼에 첫 포커스.
- 키 추가: `solve.modeGroupLabel`, `solve.practiceAriaLabel`, `solve.stageProgressLabel`.

### 3-5. 드래그 로직 DRY 통합 (`1971c61`, 2026-05-23)
- 매뉴얼 입력(`scramble-cube.tsx`)이 갖고 있던 인라인 raycast / 면 감지 / 슬라이스 해석 /
  pointer 핸들러를 모두 제거하고 `useCubeDrag({ enabled, onResolveMove, onHover })` 호출로 대체.
- 매뉴얼 입력 책임은 **콜백 안 적용**만 — `onResolveMove`: `rotateCube + applyMove + setMoveHistory`,
  `onHover`: `setHoverHint`.
- canvas pointer-events/touchAction 설정 + orbit 시점 회전 + 카메라 복귀 tween 은 매뉴얼 입력에 남김.
- 파일 길이: **505 → 275줄 (-231)**. 연습 모드와 매뉴얼 입력이 한 훅을 공유하므로,
  드래그 관련 버그 수정·기능 추가가 한 곳에서 끝남.
- ⚠️ 인터랙션 코드라 자동 테스트 불가 → **브라우저 smoke test** 필요 (드래그 회전, 호버 미리보기,
  빈영역 시점 회전, 1수 적용 후 무브 히스토리 누적).

### 3-6. 2-4 연습/힌트 모드 (`31f9b6c`)
- **신규** `components/cube-visualization/use-cube-drag.ts`: 큐브 드래그→무브 해석을
  `onResolveMove(move)` 콜백으로 분리한 공유 훅. 적용은 호출부 책임.
  - DRY 통합 완료 (§3-5) — 매뉴얼 입력도 같은 훅을 사용.
...
### 개선
- **연습 모드 런타임 테스트** 후 다듬기 (드래그/힌트/180°/모드 전환 한 사이클 — 사용자 손).
- **DRY 통합 회귀 점검** — 매뉴얼 입력의 드래그 회전·호버·시점회전·히스토리 누적이
  훅 변환 후에도 동일하게 동작하는지 브라우저 smoke test (사용자 손).
- **영문 번역 다듬기** — 특히 `messages/en.json` 의 `stages.lblDesc.*` / `solve.*` 의
  어색한 문장(자연스러운 cubing 용어로 교체).
- **messages dynamic import** — 활성 언어만 로드. 효과 ~수 KB (전체 365 kB의 ~3%) 라
  우선순위 낮음 — 다른 무거운 작업 끝낸 뒤 검토.
- 모바일 터치 제스처 검증 (나중).
