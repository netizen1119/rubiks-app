...
export const fcube_to_ifcube = (fcube: string) => {
  ...
  for (let i = 0; i < edgeFacelet.length; i++) {
    for (let j = 0; j < edgeFacelet.length; j++) {
      ...
      if (fcube[edgeFacelet[i][0]] === col[0] && fcube[edgeFacelet[i][1]] === col[1]) {
        out_cube[edgeFacelet[i][0]] = edgeFacelet[j][0].toString();
        out_cube[edgeFacelet[i][1]] = edgeFacelet[j][1].toString();
        break;
      } else if (fcube[edgeFacelet[i][0]] === col[1] && fcube[edgeFacelet[i][1]] === col[0]) {
        out_cube[edgeFacelet[i][0]] = edgeFacelet[j][1].toString();
        out_cube[edgeFacelet[i][1]] = edgeFacelet[j][0].toString();
        break;
      }
    }
  }
