...
## 현재 상태
**Phase 2a 완료 (2026-05-29).** contour+9-neighbor ROI 검출 동작 확인 (6면 검출 + 9-grid 정렬 + 색 분류):
- `HANDOVER_v14.md` — **가장 최신** (edge-density → contour+9-neighbor 재작성 완료.
  dilate+findContours+approxPolyDP quad 필터 + median 크기 게이트 + 9-neighbor anchor →
  9 sticker lattice. grid-sampler 를 sticker 좌표 기반으로 대체 + 로고/grid line 어두운 픽셀
  제외 trimmed-mean. 브라우저 6면 검증 통과. 다음: Phase 2b.)
...
다음: Phase 2b — 9-tuple 색 배열 시간순 추적 → idle/moving/settled state machine →
18 무브 가설 검색 (`lib/vision/move-detector.ts` stub 채우기). HANDOVER_v14 §4.
...
5. `move-detector.ts` — 모션 게이트 + 18 가설 검색. **stub** (Phase 2b).
6. `tracker-bridge.ts` — 확정 무브 후보 → store dispatch. **stub** (Phase 3).
